...
- `(i << 6) | j` のノリをBFSでもやる
- 可読性が終わる
- N=30,K=3で80000回くらいループ(うろ覚え)
- スコアは上がらない(235550点)

#### BFSの高速化
- ループを回すのをN×NではなくK×100にする
- 反復回数はN=30,K=3で100000回を超えてきた
- あがらん……(221853点)
- ループ順が変わっているからかむしろ下がってる
- 実はその後の line_fill や can_connect のほうが重いのであまり意味ないかも


### 2022/08/15
#### calc_score_fastをさらに早くできそう
- 全然上がらねえ！
#### BFSを焼きなます
- kindとindの順番を少しずつ変えることでBFSも焼きなませるかも
- K*100の配列をちょっとずつスワップするイメージ

- そういえば結局焼きなまししてないな……

### 2022/08/16
#### BFSで既につながってるのに繋ぐのは無駄だと気づいた

- can_connect_pairを実装
- atcoder::dsuを使う
- 241590点
- 正直もう思いつかなくなってきたのでこの辺で終わりかな……
- Visualizerを見ると、大きめの連結成分1つ+その他、という感じなので、上手くその他らで同じ種類のサーバーなのに別々になっているやつをつなげられると良いと思うが方法が思いつかない
- パラメータガチャをしていました(過学習してそうで怖い)
